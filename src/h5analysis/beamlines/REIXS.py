...
class SimpleLoader(Load1d):
    ...
    def add(self,config,filename,y_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.add(self,config,filename,'',y_stream,*args,tempclass=Load1d,**kwargs)
    def subtract(self,config,filename,y_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.subtract(self,config,filename,'',y_stream,*args,tempclass=Load1d,**kwargs)
    def stitch(self,config,filename,y_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        return Load1d.stitch(self,config,filename,'',y_stream,*args,tempclass=Load1d,**kwargs)
    def background(self,config,filename,y_stream,*args,**kwargs):
        ...
        **kwargs
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.background(self,config,filename,'',y_stream,*args,tempclass=Load1d,**kwargs)
...
class PlotLoader(Load1d):
    ...
    def add(self,config,filename,x_stream,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.add(self,config,filename,x_stream,y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def subtract(self,config,filename,x_stream,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.subtract(self,config,filename,x_stream,y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def background(self,config,filename,x_stream,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        **kwargs
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.background(self,config,filename,x_stream,y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def stitch(self,config,filename,x_stream,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.stitch(self,config,filename,x_stream,y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def compare(self,plot_object):
        """
        Loads data previously specified in a loader
...
class ImageLoader(Load2d):
    ...
    def add(self,config,filename,x_stream,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.add(self,config,filename,x_stream,detector,*args,tempclass=Load2d,**kwargs)
    def subtract(self,config,filename,x_stream,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.subtract(self,config,filename,x_stream,detector,*args,tempclass=Load2d,**kwargs)
    ...
    def background_2d(self,config,filename,x_stream,detector,*args,**kwargs):
        ...
        **kwargs
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.background_2d(self,config,filename,x_stream,detector,*args,tempclass=Load2d,**kwargs)
    def stitch(self,config,filename,x_stream,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.stitch(self,config,filename,x_stream,detector,*args,tempclass=Load2d,**kwargs)
...
class StackLoader(Load3d):
    ...
    def add(self,config,filename,scan_stream,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load3d.add(self,config,filename,scan_stream,detector,*args,tempclass=Load3d,**kwargs)
    def subtract(self,config,filename,scan_stream,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load3d.subtract(self,config,filename,scan_stream,detector,*args,tempclass=Load3d,**kwargs)
    def plot(self,plot_width=900,plot_height=600,**kwargs):
        """
        Plot all data assosciated with class instance/object.
...
class MeshLoader(LoadHistogram2d):
    ...
    def load(self,config,filename,x_stream,y_stream,z_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        LoadHistogram2d.load(self,config,filename,x_stream,y_stream,z_stream,*args,**kwargs)
    def add(self,config,filename,x_stream,y_stream,z_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        LoadHistogram2d.add(self,config,filename,x_stream,y_stream,z_stream,*args,tempclass=LoadHistogram2d,**kwargs)
    def subtract(self,config,filename,x_stream,y_stream,z_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        LoadHistogram2d.subtract(self,config,filename,x_stream,y_stream,z_stream,*args,tempclass=LoadHistogram2d,**kwargs)
    def stitch(self,config,filename,x_stream,y_stream,z_stream,*args,**kwargs):        
        ...
        """
        filename=hdf5FileFixer(filename)
        LoadHistogram2d.stitch(self,config,filename,x_stream,y_stream,z_stream,*args,tempclass=LoadHistogram2d,**kwargs)
    def plot(self,**kwargs):
        ...
        kwargs
            all bokeh figure key-word arguments
        """
        LoadHistogram2d.plot(self,plot_width=900,plot_height=600,**kwargs)
...
class XESLoader(Load1d):
    ...
    def stitch(self,config,filename,detector,*args,grid=[None,None,None],**kwargs):
        ...
        if(len(data_args) > 1):
            ...
            x_stream='[None]'
        Load1d.stitch(self,config,filename,x_stream,detector,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def add(self,config,filename,detector,*args,grid=[None,None,None],**kwargs):
        ...
        if(len(data_args) > 1):
            ...
            x_stream='[None]'
        Load1d.add(self,config,filename,x_stream,detector,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def subtract(self,config,filename,detector,*args,grid=[None,None,None],**kwargs):
        ...
        if(len(data_args) > 1):
            ...
            x_stream='[None]'
        Load1d.subtract(self,config,filename,x_stream,detector,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def background(self,config,filename,detector,*args,grid=[None,None,None],**kwargs):
        ...
        **kwargs
        ...
        if(len(data_args) > 1):
            ...
            x_stream='[None]'
        Load1d.background(self,config,filename,x_stream,detector,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def compare(self,plot_object):
        """
        Loads data previously specified in a loader
...
class XASLoader(Load1d):
    ...
    def add(self,config,filename,y_stream,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.add(self,config,filename,'Energy',y_stream,*args,tempclass=Load1d,**kwargs)
    def subtract(self,config,filename,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.subtract(self,config,filename,'Energy',y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def background(self,config,filename,y_stream,*args,grid=[None,None,None],**kwargs):
        ...
        **kwargs
        ...
        """
        filename=hdf5FileFixer(filename)
        Load1d.background(self,config,filename,'Energy',y_stream,*args,grid_x=grid,tempclass=Load1d,**kwargs)
    def compare(self,plot_object):
        """
        Loads data previously specified in a loader
...
class XESMapper(Load2d):
    ...
    def add(self,config,filename,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.add(self,config,filename,'Energy',detector,*args,tempclass=Load2d,**kwargs)
    def stitch(self,config,filename,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.stitch(self,config,filename,'Energy',detector,*args,tempclass=Load2d,**kwargs)
    def subtract(self,config,filename,detector,*args,**kwargs):
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.subtract(self,config,filename,'Energy',detector,*args,tempclass=Load2d,**kwargs)
    ...
    def background_2d(self,config,filename,detector,*args,**kwargs):
        ...
        **kwargs
        ...
        """
        filename=hdf5FileFixer(filename)
        Load2d.background_2d(self,config,filename,'Energy',detector,*args,tempclass=Load2d,**kwargs)
...
class SPEC_HDF5():
    ...
    def load(self,dir_path,header_file):
        ...
        ----------
        dir_Path: string
            relative or absolute path to ASCII files
        header_file: string
            filename of base ASCII file
        """
        self.datasets=[[]]
        self.scanNumbers=[]
    ...
    def write_hdf5(self,mono=[],epu=[],ring=[],overwrite=False,name=None):
    ...
    def save_mca(self,filename,mca_datasets,mca_scales,mca_scanNumbers,data_group,data_name,scale_name):
        ...
        filename: string
            relative or absolute path to ASCII files
        mca_datasets: string list
            list of data read from ASCII file as strings
        mca_scales: string list
            scales read from ASCII file corresponding to datasets
        mca_scanNumbers: string list
            list of scan numbers corresponding to mca_datasets
        data_group: string
            location within HDF5 file to write data
        data_name: string
            name of dataset to be assigned within HDF5 file
        scale_name: string
            name of scale dataset corresponding to main dataset
        """
    ...
    def save_img(self,filename,img_datasets,img_xscale,img_yscale,img_xmotor,img_ymotor,img_scanNumbers,data_group,data_name):
        ...
        filename: string
            relative or absolute path to ASCII files
        img_datasets: string list
            list of data read from ASCII file as strings
        img_xscale: string list
            list of x-scales read from ASCII file
        img_yscale: string list
            list of y-scales read from ASCII file
        img_xmotor: string list
            list of motors used to scan x-scale read from ASCII file
        img_ymotor: string list
            list of motors used to scan y-scale read from ASCII file
        img_scanNumbers: string list
            list of scan numbers corresponding to img_datasets
        data_group: string
            location within HDF5 file to write data
        data_name: string
            name of dataset to be assigned within HDF5 file
...
class AsciiLoader(SPEC_HDF5):
    """ Class to load, parse and save REIXS ASCII data to HDF5 """
    def convert(self,directory,filename,mono=[],epu=[],ring=[],overwrite=False,name=None):
        """ Loads, parses and saves REIXS ASCII data to HDF5

        Parameters
        ----------
        directory: string
            absolute or relative directory location of files
        filename: string
            base filename of ASCII files, usually .dat extension
        **kwargs
            mono: string list
                specifies which motors/counters should be in the mono group
            epu: string list
